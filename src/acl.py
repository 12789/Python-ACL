...
class Acl(object):
    ...
    class __impl:
        ...
        def build_acl_from_json(self, filename):
            """Build acl from an json file."""
            ...
            role_nodes = data.get('roles')

            for role_node in role_nodes:
                roles_to_create[role_node.get('name')] = []

                # Find role parents
                if role_node.get('inheritFrom'):
                    roles_to_create[role_node.get('name')] = [
                        role_node.get('inheritFrom')]
            ...
            permission_nodes = data.get('permissions')

            for permission_node in permission_nodes:
                resource_nodes = permission_node.get('resources')
                role_nodes = permission_node.get('roles')
                privilege_nodes = permission_node.get('privileges')
                ...
                for resource_node in resource_nodes:
                    ...
                    for privilege_node in privilege_nodes:
                        resource.set_privilege(privilege_node)

                    for role_node in role_nodes:
                        try:
                            role = self._acl[role_node]
                        except:
                            raise AccessError(
                                'Role in permission is not defined.')

                        role.set_resource(resource)

        def build_acl(self, filename):
            ...
            for roles_node in roles_nodes:
                ...
                for role_node in role_nodes:
                    ...
                    parent_nodes = role_node.getElementsByTagName(
                        'inheritFrom')
                    ...
                    for parent_node in parent_nodes:
                        roles_to_create[role_name].append(
                            parent_node.childNodes[0].data)
            ...
            for permissions_node in permissions:
                permission_nodes = permissions_node.getElementsByTagName(
                    'permission')
                for permission_node in permission_nodes:
                    resource_nodes = permission_node.getElementsByTagName(
                        'resource')
                    role_nodes = permission_node.getElementsByTagName('role')
                    privilege_nodes = permission_node.getElementsByTagName(
                        'privilege')
                    ...
                    for resource_node in resource_nodes:
                        ...
                        for privilege_node in privilege_nodes:
                            resource.set_privilege(
                                privilege_node.childNodes[0].data)
...
if __name__ == '__main__':
    ...
    # Build ACL from json config file
    acl = Acl()
    acl.build_acl_from_json('permissions.json')

    assert(acl.check_access('manager', 'time_card', 'write'))
    assert(acl.check_access('manager', 'time_card', 'approve'))
